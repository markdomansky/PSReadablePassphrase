...
Describe "Testing PSReadablePassphrase Module" {

    Describe "Import-Module" {
        Context "Function Exists" {
            It "Should Return Null" {
                import-module ..\module\psreadablepassphrase.psd1
            }
            # InModuleScope "PSReadablePassphrase" {
    ...
    Describe "Get-ReadablePassphrase" {
        Context "Function Exists" {
            It "Should exist" {
                get-command get-readablepassphrase -module psreadablepassphrase | should -not -be $null
            }
        }
        Context "Generates Passwords" {
            It "Should Produce output" {
                (get-readablepassphrase | measure).count | should -be 1
            }
            It "Should Produce secure output" {
                (get-readablepassphrase -assecurestring).gettype().name | should -be "SecureString"
            }
        }
    }
